...
const VerJustificantes = () => {
    ...
    const fetchSolicitudes = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/v1/solicitudes/?clave_empleado=${userData.clave}`, {
    ...
    return (
        <SafeAreaView style={styles.container}>
          ...
          <View style={styles.header}>
            ...
            <TouchableOpacity onPress={fetchSolicitudes} style={styles.reloadButton}>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e8ecf4' }}>
      <View style={{ flex: 1, backgroundColor: '#e8ecf4' }}>
        <View style={styles.card}>
          <Text style={styles.titulo}>Numero de empleado: {solicitud.clave_empleado}</Text>
          <Text style={styles.subtitulo}>Fecha: {solicitud.dia_justificar}</Text>
          <Text style={styles.subtitulo}>Motivo: {solicitud.motivo}</Text>
          <Text style={styles.subtitulo}>Estado: {solicitud.estado_solicitud === 'A' ? 'Aceptada' : solicitud.estado_solicitud === 'R' ? 'Rechazada' : 'Pendiente'}</Text>
        </View>
      </View>
    </SafeAreaView>
  );
